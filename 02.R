...
count_colors = function(string_list){
  count_store = list(green = 0 , red = 0 , blue = 0 )
  color_strings = paste(string_list , collapse = ',') |> 
    strsplit(',') 
  color_strings |> 
    sapply(function(x){
      strsplit( trimws(x) , ' ') |> 
        sapply(function(x){
          count = x[[1]] |> as.numeric()
          color = x[[2]] 
          if( color == 'green' ) count_store$green <<- max(count_store$green , count )
          if( color == 'red' ) count_store$red <<- max(count_store$red , count )
          if( color == 'blue' ) count_store$blue <<- max(count_store$blue , count )
        })
    })
...
reds_ok = count_df$red <= 12
greens_ok = count_df$green <= 13
blues_ok = count_df$blue <= 14

all_ok = reds_ok & greens_ok & blues_ok
ok_indices = which(all_ok)
sum(ok_indices)

apply(count_df , 1 , function(x) prod(unlist(x)) ) |> sum()
