...
replace_alphabet_digits = function(string){
  string %>% 
    gsub('one' ,  'one1one', .) %>% 
    gsub('two' ,  'two2two', .) %>% 
...
extract_digits = function(string){
  digits = gsub("[a-z]", "", string  )
  first_digit = substr(digits, 1, 1) 
  last_digit = substr(digits, nchar(digits), nchar(digits)) 
  return( paste0(first_digit , last_digit) %>% as.numeric() )
}

input_string %>% 
  textConnection() %>%
  readLines() %>%
  sapply(replace_alphabet_digits) %>%
  extract_digits() %>%
  sum()
